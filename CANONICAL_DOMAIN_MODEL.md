...
| AI System Governance Enrichment | ai_systems (extended, migration 20261037) | PATCH /api/ai-systems/:id (business_owner_user_id, eu_ai_act_tier, human_oversight_level, sensitive_data_categories, review_cadence_days, next_review_due) | T2-C/T2-D — HELD branch feat/ai-t2-governance-model. `human_oversight_level` is the vendor_engagements.ai_autonomy vocabulary VERBATIM (drift pinned by aiGovernanceVocabulary.test.ts); model provider is NOT a column — it is the existing ai_system_vendor_dependencies edge with dependency_role='model_provider'. Material change: PATCH diffs load-bearing fields BY VALUE, bumps material_state_version, records reassessment_recommended_at/_reason; overdue computed at read. |
| AI System Governance Links | ai_system_framework_links, ai_system_control_links, ai_system_policy_links, ai_system_obligation_links (migration 20261038) | POST /api/ai-system-{framework,control,policy,obligation}-links, DELETE .../:id, GET /api/ai-systems/:id/{frameworks,controls,policies,obligations} | T2-B — HELD branch feat/ai-t2-governance-model. Four TYPED edges (never polymorphic); human declarations only; both endpoints pre-flighted same-org; create/delete only (no UPDATE grant — re-pointing is delete+create, two audit rows); RLS + app_request grants at creation. |
| AI Use Approval | ai_use_approvals (migration 20261039) | POST /api/ai-systems/:id/use-approvals, GET /api/ai-systems/:id/use-approvals | T2-D2 — HELD branch feat/ai-t2-governance-model. APPEND-ONLY formal use decisions (approved / approved_with_conditions / rejected / suspended), modeled on the ratified vendor_engagements decision block; rationale DB-enforced, conditions iff conditional, expiry only on approval; snapshots material_state_version so "approved but materially changed since" is computable; current decision = latest row; no UPDATE/DELETE grant. |
