...
| Vendor Engagement Finding — supersede-on-pass observation | (no table — DERIVED at read: findings × requirement_responses) | POST /api/vendor-engagements/:id/promote-findings (`superseded_by_source` in response + audit), GET /api/vendor-engagements/:id (`findings` block) | RULED 2026-08-22, HELD branch `feat/va-supersede-on-pass` (sequenced after #863). **A pass supersedes nothing automatically** — fourth appearance of machines-observe-humans-decide (scanner reappearance never reopens; monitoring recommendation never transitions; pen-test retest never closes; engagement pass never closes). The machine NAMES every open engagement finding whose control's current response is `pass` or `not_applicable` (labeled distinctly — closing on "we do it" vs "does not apply" are different human judgments) and derives the fact fresh at read; closure stays with the human gate (SoD, decision_state). **Cross-engagement (ruled 2026-08-23):** the observation spans every engagement of the same vendor — a new engagement never makes an earlier engagement's finding invisible; equivalence is deterministic-only (vendor_id + requirement_id FK identities, no text matching), each named row carries `source_engagement_id`, and findings with NULL `requirement_id` are surfaced as `supersede_equivalence_undetermined` rather than guessed at. Deliberately NO stored marker and NO finding_lifecycle_events widening (that ledger is transitions-only). CUEC arm needs no analogue: its 409 `cuec_already_promoted` guard BLOCKS the divergence instead — though that guard ignores whether the finding was since closed (follow-up filed). |
| Vendor Contact | vendor_contacts (migration 20261056) + vendor_engagement_invites.contact_id | GET /api/vendors/:id/contacts, POST /api/vendors/:id/contacts, PATCH /api/vendors/:id/contacts/:contactId, DELETE /api/vendors/:id/contacts/:contactId; addressee on POST /api/vendor-engagements/:id/issue and .../reissue-invite | VA-C1 — HELD branch `feat/va-c1-vendor-contacts` (stacked on #878). **A PERSON AT A SUPPLIER, NOT A USER AND NOT A PARTICIPANT.** No account, no login, no session: a data subject whose PII we hold without them ever signing in (Category C, piiRisk high), so an erasure request can arrive from someone unfindable through `users`. Standing relationship fact — participation in one assessment is a SEPARATE record (VA-P1), or removing somebody from a questionnaire would delete them from the supplier's directory. Identity = (organization_id, vendor_id, lower(email)); one primary per supplier among ACTIVE rows only (partial unique index), and promoting demotes rather than 409-ing. **Contacts DEACTIVATE, never delete** once used: DELETE is refused (409) while any invite references the row, because their name is attached to answers and evidence that outlive their employment. The invite KEEPS its own contact_email/contact_name snapshot and the FK is ON DELETE SET NULL — editing a contact must never rewrite who was historically invited. Raw-address issuance stays supported (contact_id is optional). **Owner ruling 2026-08-23 (criticality vs tier):** this table adds NO criticality or tier column — enduring relationship significance is `vendors.criticality`, the depth of one assessment is `vendor_engagements.assessment_tier`, and the two are never collapsed; the engagement read now carries `vendor_criticality` so both are visible side by side. |
| Engagement Participant | vendor_engagement_participants (migration 20261057) + vendor_engagement_invites.participant_id | GET /api/vendor-engagements/:id/participants, POST /api/vendor-engagements/:id/participants, POST /api/vendor-engagements/:id/participants/:participantId/revoke; vendor-side GET/POST /api/vendor-portal/participants, POST /api/vendor-portal/participants/:participantId/revoke | VA-P1 — HELD branch `feat/va-p1-vendor-participants` (stacked on #879). **WHO MAY WORK ON ONE ASSESSMENT — not a person, and not a contact.** Points at `vendor_contacts` (VA-C1) and never copies it: removing somebody from a questionnaire must not delete them from the supplier directory, so `contact_id` is ON DELETE RESTRICT. Identity = (organization_id, engagement_id, contact_id) — re-inviting REUSES the row, because two rows for one human would mean two sets of attribution. **Attribution adds NO columns:** the four existing `*_via_invite_id` provenance columns (requirement_responses, requirement_response_revisions, evidence, vendor_engagement_comments) resolve to a person through invites.participant_id, which is written once at mint and never re-pointed — so a later contact edit cannot rewrite who authored what, and there is no second place for authorship to disagree with itself. **Roles are two, deliberately** (coordinator / contributor, no RBAC): the coordinator is the recipient the customer addressed the assessment to, may invite and revoke teammates, and is the ONLY role that may SUBMIT — submission freezes the questionnaire and ending everyone's work is not a permission a contributor was granted. One live coordinator per engagement (partial unique index over non-revoked rows). **The single-active-invite rule became PER PARTICIPANT** (partial unique index on live participant invites): re-issuing to one person kills only their credential and sessions; the engagement-wide sweep would have evicted the team. **Revocation = access ends, history stays** (status + revoked_at; the row is never deleted; the portal resolver treats participant revocation as session revocation on the very next request). A pre-VA-P1 session has no participant record and is treated as the sole recipient, keeping its submission authority. Concurrency: the answer upsert stays last-write-wins but now carries an OPTIONAL `prev_answered_at` precondition (412 `answer_changed`) so colleagues cannot silently overwrite each other. Cross-engagement isolation is the boundary org scoping and RLS give no help with, and is enforced by engagement-scoped lookups plus a trigger asserting engagement.vendor = participant.vendor = contact.vendor. |
| Engagement Work Assignment | vendor_engagement_assignments (migration 20261058) | GET/PUT /api/vendor-portal/assignments[/:requirementId], GET /api/vendor-portal/assignments/:requirementId/history, POST /api/vendor-portal/assignments/framework, GET /api/vendor-portal/progress, GET /api/vendor-engagements/:id/progress | VA-D1 — HELD branch `feat/va-d1-questionnaire-delegation` (stacked on #880). **WHO IS EXPECTED TO DO THE WORK — which is not who did it.** ASSIGNED TO (this table) and ANSWERED BY (requirement_responses.answered_via_invite_id -> invite -> participant) are different facts and both survive: a coordinator answering a question assigned to Susan records an answer by the coordinator and leaves the assignment pointing at Susan. **APPEND-ONLY LEDGER** — reassignment stamps `superseded_at` and inserts; the current assignment is the single row with superseded_at IS NULL (partial unique index), so ownership history is reconstructable from the table rather than only from fire-and-forget audit rows. `assigned_to_participant_id` NULL is a real state with `assignment_action` saying why; a revoked assignee's work is VACATED and awaits a coordinator, **never auto-given to a guessed successor**. **NOT on vendor_engagement_scope_items** (owner ruling): that table is the frozen issued scope and must stay immutable. **Sections = framework_id** (owner ruling 2026-08-23): `scope_tags` is a multi-valued applicability vocabulary derived heuristically from `reference_id || ' ' || title`, and `reference_id` is unvalidated free text — neither is an authoritative taxonomy, and inferring one from display text was refused. A framework action is a READ-SIDE BULK expansion into per-requirement rows; **no section object is stored**, so a richer taxonomy later changes only the selection layer. A single-framework engagement returns `framework_grouping_available: false` and refuses bulk assignment (422) rather than presenting one group that silently means everything. **Completion is DERIVED, never stored** — same condition as the submit guard, one denominator for overall/per-framework/per-participant figures; `clarification_open` is a SEPARATE fact (a reviewer comment on that question newer than the vendor's last answer), because "has an answer" and "needs rework" must not collapse. Customer sees progress SHAPE + contributors, never the supplier's per-question delegation map. |
