...
| Assessment Fact | `assessment_facts` (migration `20261063`, VA-Q2 P3 — PLANNED, not built) | GET/PUT `/api/vendor-engagements/:id/facts` (P3; internal-only, entitlement-gated, no portal surface) | VA-Q2 — **D1 ruled Option B 2026-08-28** (see Locked Package Decisions). **THE ONE canonical fact object.** A declared fact about a canonical *subject* — `(organization_id, subject_type, subject_id, fact_key)` — with `value` JSONB + `value_hash`, `source` (trust class) + `origin` (mechanism), `provenance` JSONB, `observed_at` / `verified_at`, `confidence`, `status` and a `supersedes_id` history chain; values are immutable (new value = new row, prior row `superseded`), app_request has no DELETE. `subject_type` is a **CLOSED allowlist** (DB CHECK + code enum, lockstep-tested); Q2 accepts `vendor_engagement` only. Polymorphic `subject_id` has no FK: integrity = per-subject-type resolver inside the tenant scope comparing `organization_id` + BEFORE INSERT/UPDATE subject-existence/org trigger + RLS by `organization_id`. `fact_key` must exist in the code fact registry (`FACT_REGISTRY`, versioned with `SCOPE_RULE_VERSION`). The 13 inherent-risk columns on `vendor_engagements` remain the store of record for inherent risk and are *mirrored* in (`origin='intake'`). Read by the deterministic scope resolver (S2/S5) — `accepted` rows only. Never a second per-subject fact table (facts for vendors / AI systems / assets / organizations widen the allowlist, they do not add tables). |
...
### Fact Subject Type (assessment_facts.subject_type CHECK constraint — CLOSED allowlist)
Single source of truth: `FACT_SUBJECT_TYPES` in `src/api/lib/vendorRisk/factSubjects.ts`
(VA-Q2 P3); the DB CHECK mirrors it and a lockstep test reads `pg_constraint`.
A value enters this list ONLY with the migration that widens the CHECK, the
resolver arm that loads the subject inside the tenant scope, and its adversarial
tests. Never an arbitrary string.
- `vendor_engagement` → `vendor_engagements` — **ACCEPTED (VA-Q2 writer)**
- `vendor` → `vendors` — RESERVED (not accepted by any Q2 writer or the CHECK)
- `ai_system` → `ai_systems` — RESERVED
- `asset` → `assets` — RESERVED
- `organization` → `organizations` — RESERVED

### Fact Source (assessment_facts.source CHECK constraint — trust class: who asserted it)
- `intake` — the internal engagement intake (the 13 inherent inputs and the extended declarations); verified by definition
- `vendor_response` — asserted by the vendor through the portal; **never authoritative**; may widen an issued scope, never narrow it (no Q2 writer — Q3)
- `ai_extraction` — proposed by a model; born `proposed`; becomes `accepted` only through the governed human-accept boundary (no Q2 writer — Q6)
- `internal_user` — declared by an authenticated internal reviewer outside intake
- `system_derived` — mirrored/derived by the platform from canonical rows (vendor profile flags, AI-system dependencies, profile defaults)

The companion column `origin` keeps VA-Q0 §6.1's *mechanism* vocabulary
(`intake, vendor_profile, ai_system_dependency, vendor_answer, profile_default, derived`)
over which precedence is ranked; allowed `(source, origin)` pairs are a DB CHECK
(VA-Q2 plan §F P3).

### Fact Status (assessment_facts.status CHECK constraint)
- `proposed` — recorded, not admitted to the resolver's fact set (all `ai_extraction` rows start here)
- `accepted` — admitted to the resolver's fact set (means *admitted*, not *verified true* — authority is `source` + `verified_at`)
- `superseded` — replaced by a later row (`supersedes_id` on the successor); retained as history
- `rejected` — refused by a human at the accept boundary; retained as history
...
Organization
  ...
  ├── Assessment Facts (organization_id FK; polymorphic by (subject_type, subject_id) over a CLOSED allowlist — Q2: vendor_engagements only; vendor / ai_system / asset / organization RESERVED. No FK on subject_id: integrity = tenant-scoped per-type resolver + subject-existence/org trigger + RLS. One accepted value per (subject, fact_key, source, origin); history via supersedes_id. Read by the deterministic scope resolver; the 13 inherent inputs on vendor_engagements are mirrored in, never moved. VA-Q2 D1 Option B, 2026-08-28.)
...
### VA-Q2 D1 — Option B (2026-08-28)

1. **One fact object, subject-addressed**: `assessment_facts` is the canonical polymorphic fact model, keyed by an explicit `subject_type + subject_id` relationship plus `organization_id`. Owner rationale: facts are broader than one engagement; they must represent facts about canonical subjects over time (Security, Privacy, AI, Fourth/Nth party, reassessment, continuous monitoring, future service/product/AI-system contexts). It is NOT an unconstrained generic polymorphic table. VA-Q0 §4.3's `engagement_facts` is superseded by this ruling (read as `assessment_facts` with `subject_type='vendor_engagement'`); ADR-0013 R1–R6 are unaffected.

2. **Closed subject allowlist**: `subject_type` is a DB CHECK **and** a code enum, lockstep-tested. Q2 accepts `vendor_engagement` only; `vendor`, `ai_system`, `asset`, `organization` are RESERVED and refused until each ships with its own migration, resolver arm and adversarial tests. Never a second per-subject fact table.

3. **Integrity without FK**: every write and read (a) establishes tenant ownership, (b) resolves the subject through the per-type resolver inside the tenant RLS scope and compares `organization_id`, (c) is re-checked by a BEFORE INSERT/UPDATE trigger that verifies the subject exists in the same organization, and (d) is bounded by RLS on `organization_id`. No caller may supply org A + a subject of org B and succeed on create, read or update — each layer has a named test.

4. **Authority**: vendor-sourced facts are never authoritative truth; `verified_at` is set only by internal verification; during an ISSUED assessment vendor facts may widen scope but never narrow it; narrowing happens only on a fresh, governed reassessment (new engagement) from verified facts; AI-derived facts are born `proposed` and become `accepted` only through the existing human-accept boundary. Provenance and source are always distinguishable on the row.

5. **Registry, idempotency, history**: `fact_key` must exist in the versioned code registry; dedup is a unique idempotency key over (org, subject, fact_key, value_hash, source, origin); values are immutable — a new value is a new row that supersedes the old; `app_request` has no DELETE.

6. **D2 — scope items only**: Q2 stamps the assessment domain on `vendor_engagement_scope_items` only; no finding-domain expansion (a finding's assessment domain is derived at read).
