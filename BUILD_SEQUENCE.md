...
> **Doc-sync 2026-07-03 (e) — Priority 4 B/C/D reconciliation (BUILD_SEQUENCE.md only; no application code changed by this sync).** This doc had frozen its P4 record at slice A3 (2026-06-26) while the **entire remaining plan chain — 4B (source qualification), 4C (clustering), 4D (provenance) — shipped to `develop`** in ten additive, flag-gated commits. This sync records them as **shipped-to-`develop`, INERT (all three flags unset in every env), NOT promoted to `main`** (production `main` `959951b9` still carries only the 4A.1 contract stubs). It also records two open engineering items (the **C2b `cluster_key` persistence gap** and the **owed 4C fingerprint-validation spike**) and the deferred, plan-optional **A4**. No code/tests/config/migrations changed by this sync. Evidence (VERIFIED): each commit confirmed an ancestor of `develop` via `git merge-base --is-ancestor`; flags confirmed absent from `render.yaml` and `.env.example`; the C2b gap confirmed by reading the ingestion INSERT column list at `briefScheduler.ts:165-186`.

- **Priority 4 — Phase 4B (source qualification, D3) — COMPLETE (merged to `develop`; INERT; NOT on `main`).** Flag `SECURELOGIC_SOURCE_QUALIFICATION_ENABLED` (unset → off). VERIFIED commits on `develop`: **B1** global `sources` table storage (`563e2bfb`, `db/migrations/20260707_sources.sql`); **B2** static authority annotations (`f46a2ba2`, `20260708_sources_authority.sql` + `src/api/lib/signals/sourceAuthority.ts`); **B3** rolling reliability scorer from `feed_health` (`0ee701a6`, `src/api/lib/signals/sourceReliability.ts`; per-cycle recompute wired at `briefScheduler.ts:266`); **B4** flag-gated qualification ranking (`790208f8`; `briefScheduler.ts:259-273` swaps to `makeQualificationPriority(...)` only when `sourceQualificationEnabled()`). Global signal-layer (no `organization_id`). Flag-off ⇒ byte-identical brief ordering.

- **Priority 4 — Phase 4C (multi-signal clustering, D2) — COMPLETE with one open gap (merged to `develop`; INERT; NOT on `main`).** Flag `SECURELOGIC_SIGNAL_CLUSTERING_ENABLED` (unset → off). VERIFIED: **C1** `clusterKey()` soft-corroboration fn (`89046a51`, `src/api/lib/signals/clusterKey.ts`); **C2** additive `cluster_key` column + partial index (`b4a10ec6`, `20260709_cyber_signals_cluster_key.sql`; computed in `cyberSignalNormalizer.ts:201`, **beside** `dedup_hash`, never touching it or its unique indexes); **C3a/C3b** flag-gated CVE-less fingerprint bucketing in the brief (`da41a9a4`, `19eadbf5`; `intelligenceBriefGenerator.ts:444,706`). ⚠ **OPEN — C2b persistence gap:** the ingestion INSERTs (`briefScheduler.ts:165-186`, `services/intelligence-worker/src/pipeline/runPipeline.ts`, `services/intelligence-worker/src/kevPoller.ts`, and the `src/api/routes/cyberSignals.ts` inserts) do **not** persist the normalizer-computed `cluster_key`, so live-ingested signals carry NULL and clustering would silently no-op at flag-on; only the manual `scripts/backfill-cluster-key.ts` stamps it. **C2b (persist the computed value into every ingestion INSERT) is the next code slice.** ⚠ **OPEN — validation debt:** plan §10 decision 2 requires validating the CVE-less fingerprint against real historical signals in a 4C spike **before** wiring into brief generation; C3b was wired without that spike, so the spike is owed retroactively before any `develop → main` promotion.

- **Priority 4 — Phase 4D (provenance references, D5) — COMPLETE (merged to `develop`; INERT; NOT on `main`).** Flag `SECURELOGIC_SIGNAL_PROVENANCE_ENABLED` (unset → off). VERIFIED: **D1** org-scoped `brief_item_signal_provenance` edge table with RLS (NOT FORCE) + NULLIF policy (`d6232569`, `20260710_brief_item_signal_provenance.sql`); **D2** flag-gated provenance-edge population during brief generation (`569a1fec`; `briefScheduler.ts:459-477`). Per-org (`organization_id NOT NULL`) — the one P4 slice on the per-org side; RLS-ready, inert pre-flip.

- **Priority 4 — A4 (runtime registry resolution / fan-out unification) — NOT STARTED (deferred, plan-optional).** Plan §10 decision 6 defers A4 unless the registry work surfaces a concrete need; `briefScheduler.ts` still imports and calls each direct adapter's fetch function directly and nothing consumes `API_SOURCES` outside its test. Not required for B/C/D to function.
