...
import { ThreatModelItem, ThreatModelResult } from "../../types/v2/ThreatModel";
...
export class ThreatModelEngine {
  ...
  static build(scoring: ScoringResult): ThreatModelResult {
    const items = scoring.scored.map(s => this.toThreatItem(s));
    return { items };
  }

  private static toThreatItem(s: ScoredControl): ThreatModelItem {
    return {
  ...
  private static mapSeverity(s: ScoredControl): number {
    const risk = s.risk;
    if (risk >= 15) return 5;
    if (risk >= 10) return 4;
    if (risk >= 6)  return 3;
  ...
  private static mapScenario(s: ScoredControl): string {
