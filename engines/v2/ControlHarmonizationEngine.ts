import { CanonicalControl } from "../../types/Control";
 ...
 */
export class ControlHarmonizationEngine {

  /**
   * Group and merge controls by canonicalId.
   */
  static harmonize(controls: CanonicalControl[]): CanonicalControl[] {
    const map = new Map<string, CanonicalControl>();
    ...
    for (const c of controls) {
      if (!map.has(c.canonicalId)) {
        ...
        existing.frameworks = Array.from(
          new Set([...(existing.frameworks || []), ...(c.frameworks || [])])
        );
