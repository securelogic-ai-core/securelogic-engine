...
/**
 * RisksMitigatedCard (RR-4) — read-only sidebar card on the control detail
 * page. Lists the risks this control mitigates. The add/remove affordance
 * lives on the risk side (LinkedControlsSection) — risk owners decide
 * which controls mitigate their risks; this view is the inverse readout.
 *
 * Mirrors FrameworkMappingsCard's outer shape (rounded card, uppercase
 * header + count badge, footer link). Per-row layout simplified to:
 * residual-rating dot · title · status badge · subtitle.
 */
...
export function RisksMitigatedCard({ controlId }: { controlId: string }) {
  const [links, setLinks]     = useState<ControlRiskLink[]>([]);
