...
/**
 * HistorySection — chronological audit trail for a single register
 * object. The RR-3 RiskHistorySection promoted to a shared component
 * (the ExportCsvButton pattern): the engine serves
 *   GET /api/<resourcePath>/:id/history
 * from security_audit_log (object + its satellite resources), and this
 * renders it. Vendors, controls, obligations, AI systems, and risks all
 * share this one implementation.
 *
 * Fetches client-side via the Next.js proxy at
 *   /api/<resourcePath>/[id]/history?limit&offset
 ...
 * belongs to this object or one of its satellites — redundant here).
 */

import { useEffect, useState } from "react";
import { getResourceHistory, type AuditLogEvent } from "@/lib/api";
...
export function HistorySection({
  ...
  resourcePath: HistoryResourcePath;
  resourceId: string;
}) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    getResourceHistory(resourcePath, resourceId, { limit: PAGE_SIZE, offset: 0 })
      .then((res) => {
        if (cancelled) return;
        if (!res) {
  ...
  async function loadMore() {
    setLoadingMore(true);
    const res = await getResourceHistory(resourcePath, resourceId, {
      limit: PAGE_SIZE,
      offset,
    });
