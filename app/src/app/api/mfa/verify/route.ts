...
export async function POST(request: Request) {
  const body = await request.json() as { code?: unknown; mfa_token?: unknown };

  const res = await fetch(`${ENGINE_URL}/api/auth/mfa/verify`, {
    method:  "POST",
    headers: { "Content-Type": "application/json" },
    body:    JSON.stringify({ code: body.code, mfa_token: body.mfa_token }),
    cache:   "no-store"
  });
  ...
  // Successful MFA — set the session cookie, same as auth-login does
