...
import { updateGovernanceAssessmentStatus, type UpdateGovernanceAssessmentStatusResult } from "./assess/actions";
import type { AiGovernanceAssessment } from "@/lib/api";
...
const ALL_STATUSES: Array<{ value: AssessmentStatus; label: string }> = [
...
const FINDING_TRIGGER_STATUSES = new Set<AssessmentStatus>(["non_compliant", "partially_compliant"]);
...
export function AssessmentStatusCard({ assessment, systemId }: Props) {
  ...
  const needsSeverity = FINDING_TRIGGER_STATUSES.has(optimisticStatus);
  const transitionTargets = ALL_STATUSES.filter((s) => s.value !== optimisticStatus);

  function handleTransition(newStatus: AssessmentStatus) {
    const previous = optimisticStatus;
    setOptimisticStatus(newStatus);
    setError(null);
    startTransition(async () => {
      const result = (await updateGovernanceAssessmentStatus(
        assessment.id,
        newStatus,
        selectedSeverity,
        systemId
      )) as UpdateGovernanceAssessmentStatusResult | void;
