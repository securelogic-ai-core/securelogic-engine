...
import { importObligations, type ObligationImportRow, type ObligationImportResult } from "./actions";

// ─────────────────────────────────────────────────────────────
// Constants
// ─────────────────────────────────────────────────────────────

const FIELDS: Array<{ key: keyof ObligationImportRow; label: string; required?: boolean }> = [
  { key: "title",             label: "Title",                  required: true },
  { key: "description",       label: "Description" },
  { key: "source_regulation", label: "Source Regulation" },
...
function matchDomain(raw: string | undefined): string | undefined {
  if (!raw) return undefined;
  const key = raw.trim().toLowerCase().replace(/[_]+/g, " ");
  return CANONICAL_DOMAINS.find((d) => d.toLowerCase() === key) ?? raw.trim();
}
...
const TEMPLATE_FILENAME = "obligations-import-template.csv";
const TEMPLATE_HEADERS  = "title,description,source_regulation,jurisdiction,domain,status,priority,due_date,notes";
const TEMPLATE_ROW1     = '"GDPR Article 32 Compliance","Implement appropriate technical and organisational measures","GDPR","EU","Regulatory","active","immediate","2026-06-30","Annual review required"';
const TEMPLATE_ROW2     = '"Third-Party Vendor Access Review","Review and validate all vendor access rights quarterly","Internal Policy","US","Vendor Risk","active","planned","2026-09-30",""';

// ─────────────────────────────────────────────────────────────
// Auto-mapping heuristics
// ─────────────────────────────────────────────────────────────

const AUTO_MAP_RULES: Record<keyof ObligationImportRow, string[]> = {
  title:             ["title", "obligation", "name", "requirement", "obligation title"],
  description:       ["description", "desc", "details", "summary"],
  source_regulation: ["source regulation", "regulation", "source", "law", "act", "standard", "source reg"],
  jurisdiction:      ["jurisdiction", "region", "geography", "country", "location"],
  domain:            ["domain", "category", "area", "type"],
  status:            ["status", "state"],
  priority:          ["priority", "urgency", "level"],
  due_date:          ["due date", "due", "deadline", "target date", "completion date"],
...
function normalizeRow(raw: Record<string, string>, columnMap: Record<string, string>): ObligationImportRow {
  function get(field: keyof ObligationImportRow): string | undefined {
  ...
  return {
    ...
    source_regulation: get("source_regulation"),
    jurisdiction:      get("jurisdiction"),
    domain:            matchDomain(get("domain")),
...
function validateRow(row: ObligationImportRow): { status: RowValidation; warnings: string[] } {
  const warnings: string[] = [];
  if (!row.title.trim()) return { status: "invalid", warnings: ["Title is required"] };
  if (row.domain && !CANONICAL_DOMAINS.includes(row.domain)) {
    warnings.push(`Domain "${row.domain}" is not valid — will be cleared`);
  }
  if (row.status && !VALID_STATUSES.has(row.status)) {
    warnings.push(`Status "${row.status}" is not valid — will default to "active"`);
  }
...
function cleanRow(row: ObligationImportRow): ObligationImportRow {
  return {
    title:             row.title.trim(),
    description:       row.description || undefined,
    source_regulation: row.source_regulation || undefined,
    jurisdiction:      row.jurisdiction || undefined,
    domain:            row.domain && CANONICAL_DOMAINS.includes(row.domain) ? row.domain : undefined,
    status:            row.status && VALID_STATUSES.has(row.status) ? row.status : "active",
...
export function ObligationsImportClient() {
  ...
  const [previewRows, setPreviewRows] = useState<ObligationImportRow[]>([]);
  const [importResult, setImportResult] = useState<ObligationImportResult | null>(null);
  ...
  const handleImport = useCallback(async () => {
    const validRows = previewRows.filter((r) => r.title.trim()).map(cleanRow);
    setImporting(true);
    try {
      const result = await importObligations(validRows);
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <Link href="/obligations" className="inline-flex items-center gap-1.5 text-xs font-medium mb-6 transition-colors hover:opacity-80" style={{ color: "#94a3b8" }}>
        ← Obligations
      </Link>
      <h1 className="text-2xl font-bold mb-2" style={{ color: "#f1f5f9" }}>Import Obligations</h1>
      <p className="text-sm mb-8" style={{ color: "#94a3b8" }}>
        Upload a CSV or Excel file to bulk-create regulatory obligations. Maximum 500 rows per import.
      </p>
    ...
    <div style={cardStyle} className="p-6 space-y-6">
      ...
      <div>
        ...
        <textarea value={pasteText} onChange={(e) => setPasteText(e.target.value)} rows={8}
          placeholder={"title,source_regulation,domain,status,priority\nGDPR Article 32,GDPR,Regulatory,active,immediate"}
      ...
      <div className="pt-2" style={{ borderTop: "1px solid #1e2d45" }}>
        ...
        <p className="text-xs mt-1" style={{ color: "#475569" }}>Domain values: Access Management, Vendor Risk, AI Governance, Regulatory, Vulnerability, Resilience, General</p>
        <p className="text-xs mt-0.5" style={{ color: "#475569" }}>Priority values: immediate, near_term, planned, watch — Status values: active, waived, not_applicable</p>
      </div>
    </div>
  );
}
    ...
    <div className="space-y-6">
      <div style={cardStyle} className="p-6">
        ...
        <p className="text-xs mb-6" style={{ color: "#94a3b8" }}>
          We found {rawHeaders.length} column{rawHeaders.length !== 1 ? "s" : ""} and {rawRows.length} row{rawRows.length !== 1 ? "s" : ""}. Map them to obligation fields.
        </p>
...
function PreviewStep({ previewRows, importing, error, onBack, onImport }: {
  previewRows: ObligationImportRow[];
  importing: boolean;
  error: string | null;
  onBack: () => void;
  onImport: () => void;
  ...
  const importCount  = previewRows.filter((r) => r.title.trim()).length;
  ...
  return (
    <div className="space-y-6">
      <div style={cardStyle} className="px-5 py-4 flex flex-wrap gap-6">
        ...
        {invalidCount > 0 && <div><span className="text-2xl font-bold" style={{ color: "#fca5a5" }}>{invalidCount}</span><span className="text-xs ml-1.5" style={{ color: "#94a3b8" }}>will be skipped (missing title)</span></div>}
          ...
          <table className="w-full text-xs">
            <thead>
              <tr style={{ borderBottom: "1px solid #1e2d45" }}>
                ...
                <th className="text-left px-4 py-3 font-semibold" style={{ color: "#475569" }}>Regulation</th>
              ...
              {previewRows.map((row, i) => {
                ...
                return (
                  <tr key={i} style={{ borderBottom: "1px solid #1e2d45" }}>
                    ...
                    <td className="px-4 py-3" style={{ color: "#cbd5e1" }}>{row.source_regulation || "—"}</td>
                    <td className="px-4 py-3" style={{ color: "#cbd5e1" }}>{row.domain || "—"}</td>
                    <td className="px-4 py-3" style={{ color: "#cbd5e1" }}>{row.priority || "—"}</td>
                    <td className="px-4 py-3" style={{ color: "#cbd5e1" }}>{row.status || "—"}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      <div className="flex items-center gap-4">
        <button onClick={onImport} disabled={importing || importCount === 0} className="px-6 py-2 rounded-lg text-sm font-semibold transition-colors hover:opacity-90 disabled:opacity-50" style={{ background: "#00c4b4", color: "#0a0f1a" }}>
          {importing ? "Importing…" : `Import ${importCount} obligation${importCount !== 1 ? "s" : ""}`}
        </button>
...
function ResultsStep({ result, onReset }: { result: ObligationImportResult; onReset: () => void }) {
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Link href="/obligations" className="px-6 py-2 rounded-lg text-sm font-semibold transition-colors hover:opacity-90" style={{ background: "#00c4b4", color: "#0a0f1a" }}>View all obligations →</Link>
