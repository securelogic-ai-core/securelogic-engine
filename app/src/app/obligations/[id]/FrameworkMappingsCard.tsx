...
import { createObligationMapping } from "./framework-map/actions";
import type { ReadinessRequirement } from "@/lib/api";

export type ObligationMappedRequirement = {
  mappingId: string;
...
interface ObligationFrameworkMappingsCardProps {
  obligationId: string;
  mappedRequirements: ObligationMappedRequirement[];
  frameworks: FrameworkInfo[];
  allRequirementsByFramework: Record<string, ReadinessRequirement[]>;
}

export function ObligationFrameworkMappingsCard({
  obligationId,
  mappedRequirements,
  frameworks,
  allRequirementsByFramework,
}: ObligationFrameworkMappingsCardProps) {
  ...
  const filtered = frameworkRequirements.filter(
    (r) =>
  ...
  async function handleMap(requirementId: string) {
    ...
    setError(null);
    const result = await createObligationMapping(
      obligationId,
      requirementId,
      selectedFrameworkId
    );
      ...
      {mappedRequirements.length === 0 && justMappedIds.length === 0 ? (
        ...
        <div className="space-y-2 mb-3">
          {mappedRequirements.map((r) => (
            <div key={r.mappingId} className="flex items-start gap-2">
              <span
...
function RequirementPickerRow({
  ...
  return (
    ...
    >
      ...
      {isMapped && (
        <span className="flex-shrink-0" style={{ color: "#00c4b4", fontSize: "11px" }}>
          ✓
        </span>
      )}
      {isPending && (
        <span className="flex-shrink-0" style={{ color: "#475569", fontSize: "11px" }}>
          …
        </span>
      )}
    </button>
