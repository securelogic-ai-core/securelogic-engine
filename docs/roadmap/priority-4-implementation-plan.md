...
> **Execution record (2026-06-26, docs-only) — slice A3 + the A3a/A4 split.** EPIC P4-A item **A3** shipped to `develop` via PR #377 `feat/p4a-api-source-registry` (`ad8078a6`, merge commit `499650e7`), develop/staging only. **A3 was deliberately scoped to clause (1) of the A3 AC only** — register the seven direct adapters as `kind:'api'` `SourceDescriptor`s (`src/api/lib/signals/sourceRegistry.ts` + its test, +119 lines, metadata-only, inert: nothing consumes `API_SOURCES` yet). **Clause (2) of the original A3 AC — "`briefScheduler` resolves them via registry; brief fixture byte-identical" — is moved to slice A4** (registry resolution / fan-out unification), the **first runtime-integration milestone** of Priority 4 and still **deferred** (operator-confirmed split). So: **A3 = the metadata foundation; A4 = the first runtime slice.** The `briefScheduler` is unchanged by A3. The "(A4 optional debt)" framing in §8 is superseded — A4 now owns the scheduler-resolution work that A3's original AC implied. See `BUILD_SEQUENCE.md` doc-sync 2026-06-26 (d) for the full record.
