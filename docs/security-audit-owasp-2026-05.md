...
- **Admin chain is multi-layered:** `requireAdminKey` (timing-safe key compare, `requireAdminKey.ts:13-18`) + Redis-backed admin lockout (5 fails / 15-min, `adminLockout.ts:22-24`) + rate limit + audit. Live admin session cookie (`server.ts:423-432`) is correctly `httpOnly:true, secure:true`. **`requireAdminNetwork` (IP allowlist over `SECURELOGIC_ADMIN_ALLOWED_IPS`) is wired into the chain but runs in OBSERVATIONAL (dark) mode and enforces nothing** — it classifies, logs `admin_network_evaluated`, and always calls `next()` unless `SECURELOGIC_ADMIN_NETWORK_ENFORCED === "true"`, which is set in no environment. An earlier revision of this line claimed it "fails closed if `SECURELOGIC_ADMIN_ALLOWED_IPS` is empty"; that was never true of the deployed chain (the middleware had zero call sites until 2026-08-11, and has been dark since). **Do not count this control toward admin access restriction.** See `TENANT_ISOLATION_STANDARD.md` → "Staff access to customer data" for the authoritative status.
