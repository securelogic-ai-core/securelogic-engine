...
>
> **Effect on this finding (A01-G1 / E1-G1):** the cross-org isolation harness now covers the full v1 route set (13/13) — the original "narrow coverage" gap is closed for v1. A01-G1's status header is left unchanged here pending operator confirmation; it is now **eligible to flip 🟥 Open → ✅ Closed** (RLS / SQL-AST lint remain the separate durable-enforcement remediation under A04-G1, independent of test coverage).
