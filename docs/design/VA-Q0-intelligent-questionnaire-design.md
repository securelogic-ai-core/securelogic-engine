...
| I | Deterministic scoping is authoritative | §3, §6 | (exists) + Q2/Q3 | — | 13 | S1–S4 with rule_ids live; **S5 live and proven on staging across FOUR domains 2026-08-29 (`13e2e1c6`)** — `S5.security.baseline`, `S5.privacy.personal_data`, `S5.ai.declared` and `S5.nth.third_party_models` all produced items on one 1.1.0 engagement (earlier, at `9258b4fe`, only the security and nth-party arms could produce items because the corpus had no privacy- or AI-tagged requirement), every item domain-stamped, resolver still pure (no I/O) | **STAGING VERIFIED** (S1–S5) / DESIGNED (S6, E1) |
...
| DS | Dynamic scoping over the directive's fact list | §6.1 | Q2 | T-6 | 13 | **47 keys registered** (`factRegistry.ts`) and the canonical store is live: VA-Q2 P3 `assessment_facts` + `PUT/GET /vendor-engagements/:id/facts`, staging-verified 2026-08-29 (`9258b4fe`, 32/32 adversarial checks). The 13 `core.*` inputs are mirrored from the inherent-risk columns; `data.*` / `ai.*` / `nth.*` are declarable through the intake route. **Corpus curation CLOSED IN CODE 2026-08-29 (VA-Q2 P3.1, migration 20261064):** the gap was not only missing content — the tag heuristic sent all 4 NIST AI RMF functions, 6 of 8 CCPA and 3 of 12 GDPR requirements to the `core` fallback (asked as SECURITY), and mis-sent GDPR Art-12-14 to the AI domain. `curatedFrameworkTags.ts` now assigns all 24 as version-controlled reference data (privacy 17, ai 4, nth_party 1, security 2 — both deliberate), consumed by `POST /frameworks/activate`, and a requirement nothing classified is stamped `uncurated` instead of silently reading as security. **STAGING VERIFIED 2026-08-29 (`13e2e1c6`, 19/19, job-da9cej0n74is73fkv81g):** the three frameworks are activated on `[SEED] Walkthrough Org`, all 24 rows read back `curated`, and directive example 1 resolves to FOUR domains (`tier_2_high`: security 37, privacy 17, ai 4, nth_party 2) with all four S5 rule_ids firing. **VA-Q2 FULLY STAGING VERIFIED 2026-08-29**: #922 (security baseline is a protected floor, 42/42 @ `876bdcd8`) and P4 (behaviours 11/11 @ `2527a5a0`; 1.0.0 equivalence PASS @ `802f5c42` — 0 Q2 leaks, 0 regressions, all 21 goldens byte-equal). **Still owed:** SOC 2 / NIST CSF re-curation (#920), the domain-starvation ruling (#925), and the applicability record (#926, slot 20261065) | IMPLEMENTED (registry + store + S5) → DESIGNED (full directive coverage, pending corpus curation) |
