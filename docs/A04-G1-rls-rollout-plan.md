...
| `adminSubscribers.ts:42` | `GET /subscribers` (cursor branch) | LIST-ALL | `pgElevated` ‡ |
| `adminSubscribers.ts:58` | `GET /subscribers` (non-cursor branch) | LIST-ALL | `pgElevated` ‡ |
| `adminSubscribers.ts:130` | `POST /subscribers` (upsert) | SINGLE-ORG-TARGET | `withTenant(body.organizationId, …)` ‡ |
| `adminSubscribers.ts:190` | `PATCH /subscribers/:id` (`tier`/`status` mutation) | OTHER — by-PK, may be NULL-org | `pgElevated` ‡ |
| `adminSubscribers.ts:230` | `DELETE /subscribers/:id` | OTHER — by-PK, may be NULL-org | `pgElevated` ‡ |
| `adminPromoteNewsletterIssue.ts:17` | `POST /newsletter-issues/:id/promote` (read issue by PK) | OTHER — by-PK read, org unknown / may be NULL-org | `pgElevated` ‡ |
| `adminPromoteNewsletterIssue.ts:52` | `POST /newsletter-issues/:id/promote` (count active subscribers, org-specific branch) | SINGLE-ORG-TARGET (org **derived from issue row**) | `pgElevated` ‡ |
| `adminPromoteNewsletterIssue.ts:56` | `POST /newsletter-issues/:id/promote` (count active subscribers, NULL-org branch) | OTHER — NULL-org platform read | `pgElevated` ‡ |
| `adminPromoteNewsletterIssue.ts:62` | `POST /newsletter-issues/:id/promote` (flip `status`→`queued` by PK) | OTHER — by-PK write, may be NULL-org | `pgElevated` ‡ |
| `adminUpdateNewsletterIssue.ts:17` | `PATCH /newsletter-issues/:id` (read issue by PK for validation) | OTHER — by-PK read, org unknown / may be NULL-org | `pgElevated` ‡ |
| `adminUpdateNewsletterIssue.ts:78` | `PATCH /newsletter-issues/:id` (`capturePublicationContext` helper, conditional) | SINGLE-ORG-TARGET (org **derived from issue row**) | `withTenant(orgId, …)` ‡ |
| `adminUpdateNewsletterIssue.ts:89` | `PATCH /newsletter-issues/:id` (UPDATE issue by PK) | OTHER — by-PK write, may be NULL-org | `pgElevated` ‡ |
| `adminCreateNewsletterIssue.ts:54` | `POST /newsletter-issues` (organization existence check, conditional on non-null org) | SINGLE-ORG-TARGET (root-tenant `organizations`, org from body) | `pgElevated` ‡ |
| `adminCreateNewsletterIssue.ts:69` | `POST /newsletter-issues` (`capturePublicationContext` helper, conditional) | SINGLE-ORG-TARGET (org from body) | `withTenant(organizationId, …)` ‡ |
| `adminCreateNewsletterIssue.ts:72` | `POST /newsletter-issues` (INSERT issue, `organization_id` nullable) | HYBRID — NULL-org platform row OR org-specific, org from body | `pgElevated` ‡‡ |
...
‡ `subscribers` and `newsletter_issues` are both **HYBRID/NULLABLE-org** (`table-classification.md:83,113,203`): NULL org = platform-level row visible to all orgs, reachable only via the owner/elevated channel. **PR 7.c** (`adminSubscribers.ts`, squash `3f9f4a9f`): GET/PATCH/DELETE → `pgElevated` (by-PK or list-all, may hit NULL-org rows); POST → `withTenant(body.organizationId)` because the org is a required body field and the upsert is a single statement (no atomicity wrap). **PR 7.d** (`adminPromoteNewsletterIssue.ts`): all 4 sites → `pgElevated` under **Strategy A**. This is the first route where the tenant org is **derived from a fetched row** (not from the request) and may be NULL — site 17 reads the issue by PK *before* the org is known, sites 52/56 branch on `issue.organization_id` (NULL vs non-null, kept as two separate single statements — no statement mixes them), site 62 writes by PK. The least-privilege hybrid (Strategy B — `withTenant(issue.organization_id)` for the org-specific sites, elevated for the rest) was rejected: it would route the same by-PK write through two channels conditionally and cannot share a transaction across pools, for marginal benefit on an admin-gated route. Single write only (site 62) → no transaction wrap needed. Pre-existing TOCTOU at site 62 was fixed in a follow-up PR (#134): the UPDATE now guards on `WHERE id=$1 AND status='draft'` and returns 409 (`issue_state_changed`) when `rowCount=0`, i.e. when a concurrent promote flipped the issue out of `draft` between the read and the write. Disposition unchanged (still `pgElevated`); the guard does not affect the RLS channel choice. **PR 7.e** (`adminUpdateNewsletterIssue.ts` + `adminCreateNewsletterIssue.ts`): the 2 route-owned sites per file → `pgElevated` under **Strategy A** (mirroring 7.d). The existing `withTenant` blocks wrapping `capturePublicationContext` (update :78, create :69) remain unchanged — those are correctly org-scoped posture reads (`posture_snapshots`/`domain_scores`/`findings`/`actions WHERE organization_id=$1`), scoped to the org derived from the issue row (update) or body (create). `adminUpdateNewsletterIssue.ts` is structurally identical to 7.d (read-by-PK → derive org → write-by-PK). `adminCreateNewsletterIssue.ts:54` (organizations existence check) went `pgElevated` for Strategy-A consistency; the `withTenant(id)` alternative (matching `adminOrganizations.ts:99`) was considered and rejected to keep the whole route on one channel.

‡‡ `adminCreateNewsletterIssue.ts` INSERT at :72 is the first §3 entry where a **single INSERT conditionally writes either a NULL-org platform row or an org-specific row**, with the org coming from the request body (validated existence at :54) and being **optional**. The NULL case cannot route through `withTenant` (no org context to set). Strategy A (`pgElevated`) avoids a conditional dual-channel write — the simpler and more reviewable choice for a low-traffic admin POST. No atomicity wrap needed: site :54 is a read and :72 is a single statement, no multi-write orphan risk. Contrast the family: 7.c `POST /subscribers` had a **required** body org → `withTenant`; 7.d promote had org **derived from a fetched row**, by-PK → `pgElevated`; 7.e create is org-from-body but **nullable** → the NULL case forces elevated.
...
**`admin*.ts` triage — COMPLETE** (as of PR 7.e, 2026-06-02; pending set now empty). Pg-free / no action: `adminBriefs.ts`, `adminEntitlements.ts`, `adminOpsDashboard.ts`. **Done:** `adminOrganizations.ts` (PR #128), `adminOpsHealth.ts` (PR #129, all 8 → `pgElevated`), `adminApiKeys.ts` (PR 7.b, GET+DELETE → `pgElevated`, POST → single `withTenant`), `adminSubscribers.ts` (PR 7.c, squash `3f9f4a9f`, GET/PATCH/DELETE → `pgElevated`, POST → `withTenant`), `adminPromoteNewsletterIssue.ts` (PR 7.d, all 4 → `pgElevated`, Strategy A; TOCTOU fixed PR #134), `adminUpdateNewsletterIssue.ts` + `adminCreateNewsletterIssue.ts` (PR 7.e, 2 route-owned sites each → `pgElevated` Strategy A; existing `withTenant`/`capturePublicationContext` blocks unchanged). Every `admin*.ts` route file is now RLS-channel-correct. Remaining A04-G1 launch work is phase-2 (RLS policy enablement migrations) and phase-3 (`DATABASE_URL` flip to `app_request`), not per-file triage. If this inventory outgrows §3, extract to `docs/A04-G1-admin-surface-inventory.md` and leave a pointer here.
