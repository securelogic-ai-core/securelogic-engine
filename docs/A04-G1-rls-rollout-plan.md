...
**Remaining `admin*.ts` — triage pending** (the originally-heaviest list is now cleared. Partial / still-open: `adminCreateNewsletterIssue.ts` + `adminUpdateNewsletterIssue.ts` — each has 1 `withTenant` already but should be re-audited for completeness. Pg-free / no action: `adminBriefs.ts`, `adminEntitlements.ts`, `adminOpsDashboard.ts`. **Done:** `adminOrganizations.ts` (PR #128), `adminOpsHealth.ts` (PR #129, all 8 → `pgElevated`), `adminApiKeys.ts` (PR 7.b, GET+DELETE → `pgElevated`, POST → single `withTenant`), `adminSubscribers.ts` (PR 7.c, squash `3f9f4a9f`, GET/PATCH/DELETE → `pgElevated`, POST → `withTenant`), `adminPromoteNewsletterIssue.ts` (PR 7.d, all 4 → `pgElevated`, Strategy A).) If this inventory outgrows §3, extract to `docs/A04-G1-admin-surface-inventory.md` and leave a pointer here.
