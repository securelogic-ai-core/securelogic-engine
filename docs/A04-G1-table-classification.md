...
| `email_provider_events` | no | SHARED-REF | global email-provider event log (Resend webhook); writer `emailProviderWebhook.ts:74` inserts (provider, provider_event_id, event_type, email, payload) — no org column; `TENANT_ROUTE_CLASSIFICATION.md` marks touching routes tenant-irrelevant. reconciled 2026-08-11 by `20260811_email_provider_events_ddl_reconciliation.sql` (shape copied from the live production table; idempotent, so a no-op on prod). Before that it was schema-in-DB only, and the table was **absent on staging and demo** — every inbound webhook event failed there with `relation "email_provider_events" does not exist`. Production grants owner-only with RLS off; no `app_request` grant yet, which needs its own decision at the flip |
...
| `findings` | yes, NOT NULL | CUSTOMER-DATA | `organization_id` added + backfilled + `SET NOT NULL` in `20260410_platform_primitives.sql:24,51` (corrects an earlier INDIRECT classification — schema is authoritative; `assessment_id` was made nullable in the same migration). Standard per-org policy; RLS pilot in `20260619_findings_rls_pilot.sql`. Cross-org isolation harness (E1-G1) also verifies at app-layer |
...
| `sources` | no | SHARED-REF | `20260707_sources.sql` (P4/4B/B1). Global source-qualification registry. PK `source` == `feed_health.source` == registry `SourceDescriptor.id`. No `organization_id`, no RLS — same posture as `feed_health`. Seeds 13 ids (7 api + 6 rss), inert in B1 |
...
| `src/api/lib/briefEligibility.ts` (called from `briefScheduler.runScheduler` Step 1) | `SELECT id FROM organizations WHERE status='active'` then loops orgs (ADR-0007 — generation eligibility comes from org status, no longer from `intelligence_brief_subscribers`) | enumerates orgs, then per-iteration sets GUC. The enumeration itself is cross-org → elevated path for that one query; loop body runs per-org under GUC |
| `src/api/lib/briefStalenessMonitor.ts` (daily cron 08:30 UTC, `schedulerRunner.ts`) | `organizations LEFT JOIN intelligence_briefs` cross-org aggregate — newest published brief per active org (staleness observability, ADR-0007) | **must use elevated role.** Read-only outcome sweep; alerts operator webhook only |
