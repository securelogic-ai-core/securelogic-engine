...
### D-10 — iPad voice root cause pending diagnostic evidence (iOS gate temporary)
- **Status: undiagnosed — pending diagnostic evidence.** Voice transcription was reported failing on iPad, but the actual cause has **not** been determined. The earlier "WebM-only" / "iOS-produced mp4 unsupported" framing was **withdrawn — it was not evidence-based**: the client already falls back to `audio/mp4`, the engine allow-lists mp4/m4a, and Whisper accepts AAC-in-MP4, so a format mismatch is **not** a proven explanation. Candidate causes still open: **A** platform/browser capability, **B** our MIME/filename/blob handling, **C** empty/short capture, **D** transcription-endpoint rejection, **E** mic permission, **F** OpenAI/Whisper failure.
- **The blanket iOS gate is TEMPORARY** — a safe default, not a diagnosed limitation. `app/src/app/ask/voiceSupport.ts` hides the mic on production iPad/iOS; voice is launch-optional and presented as such until validated.
- **Diagnostic instrumentation (shipped, PR #420):** a non-sensitive, correlation-id-traced diagnostic captures one real attempt browser → app → engine (`app/src/app/ask/voiceDiagnostics.ts`, `src/api/lib/voiceTranscribeDiagnostics.ts`, engine event `voice_transcribe_diagnostic`). On **staging/local only** (`isStagingHost`) the iOS gate is **bypassed in diagnostic mode** so an iPad can attempt voice and surface a `VOICE-DIAG …` code + server classification; **production keeps the gate**. No audio, secrets, or PII logged.
- **Closing criteria:** this issue stays open until the diagnostic evidence is captured — **one iPad `VOICE-DIAG` browser line plus the matching engine `voice_transcribe_diagnostic` log line (same `cid`)** — the cause is classified, and the decision is applied (re-enable on capable iOS / fix our handling / defer to Realtime API / keep the gate). Full procedure, interpretation table, and decision/launch rules: `SPRINT_2.md` → **Voice Diagnostic Workstream (A-3)**.
