...
| M3 — Promotion + post-deploy verification | **4 — Production launch, COMPLETE 2026-07-21** — operator authorized merge after completing the pre-merge items (§2.3 dashboard confirmation, Sprint-3A prod URL vars); PR #688 merged as true merge `dd79330c` 18:58Z; branch invariant holds (`develop` = `main`); prod engine + app `/version` = `dd79330c` (19:00:59Z / 19:02:00Z), engine `/health` green post-65-migration auto-migrate boot; CI 8/8 on `main`; dark spot-checks clean (risk-acceptances 404, briefing-layout 401). Outstanding evidence filing (non-blocking): direct prod `schema_migrations` SELECT + deploy-log scan (operator; boot-success is the current indirect evidence), plus the §0.4 artifact appends. Briefing flag flip stays outside Sprint 1 (ruling D-E) |
