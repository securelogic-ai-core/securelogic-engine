...
6. **DEFERRED — engine Daily Digest left alive, owner investigating.** Production verification showed the live "two daily emails an hour apart" is **Brief (07:00) + Daily Digest (08:00)**, NOT the legacy Newsletter (dormant since April). The Daily Digest is a *separate, intentional product* (per-finding digest to platform `users`, preference-gated via `user_alert_preferences.daily_digest`). **Decision (owner, 2026-06-24): keep the Daily Digest alive for everyone for now — no disable, no suppression — pending further investigation.** `digestScheduler.ts` is therefore **unchanged** by this work. Consequence: the success criterion "only the Intelligence Brief can send daily customer emails" is **consciously not met** — the digest continues to send daily to platform users (subscribers and non-subscribers alike). No external customers are affected yet (all recipients are founder/demo accounts), so there is room to decide later. Options remain on the table for when the owner concludes their review: (a) leave as a separate intentional product; (b) suppress for Brief subscribers; (c) disable. Per-finding alerts (Tier 1) and the Weekly Posture Summary (Tier 3, weekly) are likewise unchanged.

## Files changed (final)

New: `src/api/lib/legacyNewsletterFeatureFlag.ts`, `src/api/__tests__/legacyNewsletterFeatureFlag.test.ts`.
Modified: `services/intelligence-worker/src/pipeline/runPipeline.ts`, `services/intelligence-worker/src/__tests__/runPipeline.test.ts`.
Unchanged (owner deferred): `src/api/lib/digestScheduler.ts` — Daily Digest left alive.
