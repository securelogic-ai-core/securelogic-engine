...
describe("decideNotification", () => {
  it("never notifies for events that don't impact the org", () => {
    expect(decideNotification({ severity: "Critical", status: "actively_exploited", customerImpacting: false }).channel).toBe("none");
  });
  ...
  it("sends immediate for a customer-impacting actively_exploited event even below Critical", () => {
    const d = decideNotification({ severity: "High", status: "actively_exploited", customerImpacting: true });
    expect(d.channel).toBe("immediate");
    expect(d.reason).toBe("customer_impacting_exploited");
  });

  it("rolls other customer-impacting events into the daily digest", () => {
    expect(decideNotification({ severity: "High", status: "corroborating", customerImpacting: true }).channel).toBe("digest");
    expect(decideNotification({ severity: "Moderate", status: "confirmed", customerImpacting: true }).channel).toBe("digest");
    expect(decideNotification({ severity: "Low", status: "mitigated", customerImpacting: true }).channel).toBe("digest");
  });
});
