...
import { getEventsList, getEventDetail } from "../routes/intelligenceEvents.js";
import {
...
describe("GET /intelligence/events/:id", () => {
  ...
  it("returns the detail when found", async () => {
    vi.mocked(getIntelligenceEventDetail).mockResolvedValue({ event: { id: "e1" }, sources: [], timeline: [] } as never);
    const res = mockRes();
    await getEventDetail(req({}, { id: "11111111-1111-1111-1111-111111111111" }), res);
    expect(res._status).toBe(200);
