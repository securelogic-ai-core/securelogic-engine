 ...
 * `subscribers` list.
 *
 * Cadence update (2026-06-26): the Brief runs every weekday (Mon-Fri) at 07:00
 * UTC. Generation and email send both run Mon-Fri; weekends are excluded at the
 * cron level AND by an in-code guard (isBriefSendDay) so manual/off-schedule
 * runs never email on a weekend. Signal ingestion (the hourly intelligence
 * worker) is unaffected by this change.
 *
...
describe("Intelligence Brief — weekday (Mon-Fri) cadence", () => {
  const SOURCE = read("../lib/schedulerRunner.ts");

  it("registers the Brief cron Mon-Fri (0 7 * * 1-5), not daily and not Monday-only", () => {
    expect(SOURCE).toMatch(/schedule\(\s*"0 7 \* \* 1-5"/);
    expect(SOURCE).not.toMatch(/schedule\(\s*"0 7 \* \* \*"/);
    expect(SOURCE).not.toMatch(/schedule\(\s*"0 7 \* \* 1"/);
  });

  it("logs the weekday schedule descriptor", () => {
    expect(SOURCE).toMatch(/schedule:\s*"0 7 \* \* 1-5 \(UTC\)"/);
    expect(SOURCE).toMatch(/description:\s*"Every weekday \(Mon-Fri\) 7:00 AM UTC"/);
  });
});

describe("Intelligence Brief — weekend email send guard (defense-in-depth)", () => {
  const SOURCE = read("../lib/briefScheduler.ts");

  it("imports the weekday send predicate", () => {
  ...
  it("skips the email send (but not generation) on a non-send day", () => {
    ...
    expect(SOURCE).toMatch(/if \(!isSendDay\) \{[\s\S]*?emails_skipped_weekend\+\+/);
    expect(SOURCE).toMatch(/event:\s*"scheduler_brief_send_skipped_weekend"/);
  });
});
