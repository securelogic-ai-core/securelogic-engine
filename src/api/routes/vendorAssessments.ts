  ...
  async (req, res) => {
    ...
    try {
      ...
      if ((vendorResult.rowCount ?? 0) === 0) {
        ...
        // Return 404 regardless of whether the vendor exists in another org
        // (prevents enumeration) or is archived.
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // source_id = assessment.id (not vendor_id) — see linkage convention
      // at top of file.
