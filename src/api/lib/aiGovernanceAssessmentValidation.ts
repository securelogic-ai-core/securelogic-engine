/**
 * aiGovernanceAssessmentValidation.ts — Pure validation for AI governance
 * assessment workflow routes.
 *
 * No I/O. Returns a discriminated union: { input } | { error, detail? }.
 *
 * ai_governance_assessments are mutable, status-driven workflow records linked
 * to ai_systems. They are distinct from governance_reviews (point-in-time,
...
// ---------------------------------------------------------------------------
// validateAiGovernanceAssessmentCreate — POST /api/ai-governance-assessments body
// ---------------------------------------------------------------------------

export type AiGovernanceAssessmentCreateInput = {
  ai_system_id: string;
  status: string;
...
export type AiGovernanceAssessmentCreateResult =
  | { input: AiGovernanceAssessmentCreateInput }
  | { error: string; detail?: string };

export function validateAiGovernanceAssessmentCreate(
  body: unknown
): AiGovernanceAssessmentCreateResult {
  ...
  const ai_system_id = b["ai_system_id"] as string;
  ...
  // overall_severity — optional, nullable (not required at POST even for
  // finding-triggering statuses; severity gate is enforced at PATCH)
...
// ---------------------------------------------------------------------------
// validateAiGovernanceAssessmentStatusTransition — PATCH body
// ---------------------------------------------------------------------------

export type AiGovernanceAssessmentStatusTransitionInput = {
  status: string;
  overall_severity: string | null;
  summary: string | null;
  notes: string | null;
...
export type AiGovernanceAssessmentStatusTransitionResult =
  | { input: AiGovernanceAssessmentStatusTransitionInput }
  | { error: string; detail?: string };

export function validateAiGovernanceAssessmentStatusTransition(
  body: unknown
): AiGovernanceAssessmentStatusTransitionResult {
  if (body === null || typeof body !== "object" || Array.isArray(body)) {
    return { error: "request_body_required" };
  }
