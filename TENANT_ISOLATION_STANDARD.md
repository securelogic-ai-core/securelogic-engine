...
### Defense-in-depth (amended 2026-08-17 via §14 — M-1 code-complete, flip pending)
Postgres Row-Level Security on customer-data tables, gated by a `app.current_org_id` session variable set per request. **Amendment (2026-08-17, M-1 PR-3 doc-sync — supersedes the 2026-07-28 text, whose "~22 tables" undercounted):** the authoritative live census (identical staging + production, read from `pg_class`/`pg_policy`) is **78 RLS-enabled tables, 78 with policies, 0 enabled-without-policy, 0 FORCE**, out of 142 public tables. The policies remain **inert** — the request path still connects as the table owner — so §§1–4's route-level discipline remains the **only live protection** until the flip. The former flip preconditions are now CLOSED in code: grant coverage is complete and CI-asserted (M1-G1 catch-up migration + the bidirectional `appRequestGrants` test, PR #802); every route/worker DB call site carries an explicit channel disposition — **tenant** (`asTenant` / `withTenant` / GUC-set explicit transaction) or **elevated** (`pgElevated`/`withElevated` with an in-situ justification) — with zero unresolved sites (PR #803); the activation gate is `scripts/validation/m1-preflight.sql` (fail-closed) and `scripts/validation/m1-proof.ts` (both-sides proof battery). Remaining is operator activation: `app_request` credential issuance, `MIGRATION_DATABASE_URL` + `DATABASE_URL` repoints per environment, staging soak, prod. Issue #695 tracks it; R8 remains open until the staging+prod flip completes.
