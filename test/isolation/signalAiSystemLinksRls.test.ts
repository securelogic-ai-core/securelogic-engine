/**
 * signalAiSystemLinksRls.test.ts — A04-G1: DB-layer RLS enforcement on
 * `signal_ai_system_links` (migration 20260702_signal_ai_system_links_rls.sql).
 *
 * Mirror of the other signal-link RLS certs. The route family
 * (signalAiSystemLinks.ts) is asTenant()-wrapped and the table is written only by
...
const INSERT_LINK = `INSERT INTO signal_ai_system_links
    (organization_id, signal_id, ai_system_id, note)
...
beforeAll(async () => {
  ...
  if (!url) throw new Error("TEST_DATABASE_URL is not set for the signal_ai_system_links RLS test.");
  pool = new Pool({ connectionString: url, ssl: false });

  signalA = await seedSignal(pool, seed.orgA.id, "rls-sasl-a");
  aiSystemA = await seedAiSystem(pool, seed.orgA.id, "SASL Org A AI system");
  signalB = await seedSignal(pool, seed.orgB.id, "rls-sasl-b");
  aiSystemB = await seedAiSystem(pool, seed.orgB.id, "SASL Org B AI system");

  const a = await pool.query(INSERT_LINK, [seed.orgA.id, signalA, aiSystemA]);
  linkA = a.rows[0].id as string;
  await pool.query(INSERT_LINK, [seed.orgB.id, signalB, aiSystemB]);
}, 120_000);

afterAll(async () => { await pool?.end(); });

describe("A04-G1 — signal_ai_system_links RLS enforcement", () => {
  it("app_request scoped to org A cannot see org B's links, and sees its own", async () => {
    ...
    try {
      ...
      const crossOrg = await client.query(
        "SELECT id FROM signal_ai_system_links WHERE organization_id = $1",
        [seed.orgB.id],
      );
      expect(crossOrg.rowCount).toBe(0);

      const visible = await client.query("SELECT organization_id FROM signal_ai_system_links");
  ...
  it("app_request scoped to org A can INSERT a link for org A (positive write)", async () => {
    ...
    try {
      ...
      await client.query("UPDATE signal_ai_system_links SET deleted_at = NOW() WHERE id = $1", [linkA]);
      const inserted = await client.query(INSERT_LINK, [seed.orgA.id, signalA, aiSystemA]);
  ...
  it("app_request scoped to org B cannot UPDATE or DELETE org A's link (rowCount 0, DB-enforced)", async () => {
    ...
    try {
      ...
      const upd = await client.query("UPDATE signal_ai_system_links SET note = 'hijacked' WHERE id = $1", [linkA]);
      expect(upd.rowCount).toBe(0);
      const del = await client.query("UPDATE signal_ai_system_links SET deleted_at = NOW() WHERE id = $1", [linkA]);
      expect(del.rowCount).toBe(0);
      const hardDel = await client.query("DELETE FROM signal_ai_system_links WHERE id = $1", [linkA]);
  ...
  it("app_request with an UNSET org GUC sees zero links (fail-closed default)", async () => {
    ...
    try {
      await client.query("BEGIN");
      await client.query("SET LOCAL ROLE app_request");
      const res = await client.query("SELECT id FROM signal_ai_system_links");
  ...
  it("app_request scoped to org A cannot INSERT a link stamped for org B (WITH CHECK)", async () => {
    ...
    try {
      ...
      await expect(
        client.query(INSERT_LINK, [seed.orgB.id, signalB, aiSystemB]),
  ...
  it("the owner connection bypasses RLS (regression — sees all orgs' links)", async () => {
    const res = await pool.query("SELECT DISTINCT organization_id FROM signal_ai_system_links");
